...
from src.dataLoaders.dataLoader import choose_data_loader
...
class Population:
    ...
    def _initialize_individuals(self, **kwargs):
        self.individuals = OrderedDict({" ": FeedForwardIndividual(
            None, self, **kwargs) for i in range(self.num_individuals)})
    ...
    def _kill_off_worst(self):
        for _ in range(
                self.num_individuals - self.death_percentage * self.num_individuals):
