import unittest 
from blacklight.individuals.individualutils import *

class TestMinLengthChromosome(unittest.TestCase):
    # Test get_min_length_chromosome
    def test_min_length_chromosome(self):
        chromosomes = {"chromosome1": {1: 1, 2: 2, 3: 3}, "chromosome2": {1: 1, 2: 2, 3: 3, 4: 4}}
...
class TestCrossoverPoints(unittest.TestCase):
    ...
    def test_crossover_points(self): 
        ...
        crossover_points = get_crossover_points_from_num_parents(num_parents, chromosome_length)
        self.assertEqual(len(crossover_points), num_parents)
        self.assertTrue(all(point < chromosome_length - 1 for point in crossover_points))
        return