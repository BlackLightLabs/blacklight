...
class TestIndividual(unittest.TestCase):
    ...
    def test_individual_random_genes(self):
        this_individual = FeedForwardIndividual(
            None, FeedForward(2, 2, 0.2, 10,))
        ...
        np.testing.assert_array_equal(
            gene_names, np.array(["gene_0", "gene_1"]))
        return
    ...
    def test_individual_random_genes_right_length(self):
        ...
        three_parent = FeedForwardIndividual(
            None, FeedForward(2, 2, 0.2, 10), NUM_PARENTS=3)
    ...
    def test_individual_crossover(self):
        ...
        population = FeedForward(2, 2, 0.2, 10)
        ...
        child_genes = {"gene_0": OrderedDict({2: "relu", 9: "relu", 10: "relu"}),
                       "gene_1": OrderedDict({11: "sigmoid", 6: "sigmoid", 7: "sigmoid"})
                       }
        ...
        for gene in child.genes.keys():
            ...
            np.testing.assert_array_equal(
                list(
                    child.genes[gene].values()), list(
                    child_genes[gene].values()))
            # Test the keys
            np.testing.assert_array_equal(
                (list(
                    child.genes[gene].keys())), (list(
                        child_genes[gene].keys())))
