...
class TestFeedForward(unittest.TestCase):
    ...
    """
    @mock.patch.object(FeedForwardIndividual,
                       '_evaluate_model', return_value=0.5)
    def test_iris_data_is_loaded(self):
        pop = FeedForward(4, 4, 0.2, 10)
        pop._evaluate = mock.Mock().method.return_value = OrderedDict(
            {FeedForwardIndividual(None, pop): 0.5, FeedForwardIndividual(None, pop): 0.6})
